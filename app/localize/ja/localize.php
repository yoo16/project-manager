<?php
require('_localize.php');

define('LABEL_NAME', '名称');
define('LABEL_LABEL', 'ラベル');
define('LABEL_LAST_NAME', '姓');
define('LABEL_FIRST_NAME', '名');
define('LABEL_LAST_NAME_KANA', 'せい');
define('LABEL_FIRST_NAME_KANA', 'めい');
define('LABEL_KANA', 'かな');
define('LABEL_AGE', '年齢');
define('LABEL_GENDER', '性別');
define('LABEL_MAIL', 'メールアドレス');
define('LABEL_MAIL_CONFIRM', 'メールアドレス（確認用）');
define('LABEL_BIRTHDAY', '誕生日');
define('LABEL_ZIP', '郵便番号');
define('LABEL_PREFECTURE', '都道府県');
define('LABEL_CITY', '市区町村');
define('LABEL_ADDRESS', '町名番地');
define('LABEL_BUILDING', '建物名');
define('LABEL_TEL', '電話番号');
define('LABEL_FAX', 'FAX');
define('LABEL_COMMENT', 'コメント');
define('LABEL_PASSWORD', 'パスワード');
define('LABEL_USER_ID', 'ユーザID');
define('LABEL_LOGIN_NAME', 'ログイン名');

define('DATABASE_NAME', 'DB名');
define('DATABASE_HOST', 'ホスト');
define('DATABASE_USER', 'ユーザ');
define('DATABASE_PORT', 'ポート');

define('LABEL_FILE', 'ファイル');
define('LABEL_SEARCH', '検索');
define('LABEL_CANCEL', 'キャンセル');
define('LABEL_BACK', '戻る');
define('LABEL_CONFIRM', '確認');
define('LABEL_SEND', '送信');
define('LABEL_RUN', '実行');
define('LABEL_SAVE', '保存');
define('LABEL_UPDATE', '更新');
define('LABEL_DELETE', '削除');
define('LABEL_EDIT', '編集');
define('LABEL_NEW', '新規');
define('LABEL_NEW_ADD', '新規追加');
define('LABEL_ADD', '追加');
define('LABEL_CREATE', '作成');
define('LABEL_INIT', '初期化');
define('LABEL_SORT_ORDER', '順番を変更');
define('LABEL_JSON', 'JSON');
define('LABEL_CSV', 'CSV');
define('LABEL_IMAGE', '画像');
define('LABEL_CLOSE', '閉じる');
define('LABEL_CLEAR', 'クリア');
define('LABEL_DUPLICATE', '複製');
define('LABEL_LIST', '一覧');
define('LABEL_LOGIN', 'ログイン');
define('LABEL_LOGOUT', 'ログアウト');
define('LABEL_REGIST', '登録');
define('LABEL_HELP', 'ヘルプ');
define('LABEL_ALL', '全て');
define('LABEL_FAVORITE', 'よく使う');
define('LABEL_SELECT', '選択');
define('LABEL_PRINT', '印刷');
define('LABEL_RELOAD', '再読み込み');
define('LABEL_SETTING', '設定');
define('LABEL_TRUE', '有効');
define('LABEL_FALSE', '無効');
define('LABEL_ERROR', 'エラー');
define('LABEL_NOTE', '備考');

define('LABEL_IMPORT', 'インポート');
define('LABEL_EXPORT', 'エクスポート');

define('LABEL_USER', 'ユーザ');
define('LABEL_STAFF', 'スタッフ');
define('LABEL_ADMIN', '管理者');

define('LABEL_BLUE', '青');
define('LABEL_RED', '赤');
define('LABEL_YELLOW', '黄');
define('LABEL_PINK', '桃');
define('LABEL_ORANGE', '橙');

define('MESSAGE_DELETE', '削除しますか？');
define('MESSAGE_CONFIRM', 'この内容でよろしいですか？');
define('MESSAGE_REQUIRED', 'を入力してください。');
define('MESSAGE_INVALID', 'が正しくありません。');