...
//TODO mysql_escape_identifier
//TODO mysql_escape_literal
//TODO mysql_field_num, mysql_field_name

class PwMysql extends PwEntity
{
    public $db_info = null;
    public $mysql_info = null;
    public $mysql_info_array = null;
    public $dbname = null;
    ...
    public static $mysql_info_columns = ['dbname', 'user', 'host', 'port', 'password'];
    ...
    {
        $this->connection();
        $values = $this->mysqli->server_version;
        return $values;
    }
    ...
    static function indexSql($conditions = null, $schema_name = 'public')
    {
        ...
        $sql = "SELECT * FROM mysql_indexes WHERE {$condition};".PHP_EOL;
        return $sql;
    }
    ...
    function loadDBInfo()
    {
        foreach (self::$mysql_info_columns as $column) {
            if (isset($this->$column)) {
                $mysql_infos[] = "{$column}={$this->$column}";
                $this->mysql_info_array[$column] = $this->$column;
            }
        }
        if ($mysql_infos) $this->mysql_info = implode(' ', $mysql_infos);
        return $this;
    }
     ...
     * @return void
     */
    function setDBInfoForString($mysql_info_string)
    {
        if (!is_string($mysql_info_string)) return;
        $mysql_infos = explode(' ', $mysql_info_string);
        if (!$mysql_infos) return;
        foreach ($mysql_infos as $mysql_info) {
    ...
    {
        $sql = "SELECT relname FROM mysql_class WHERE relkind = 'r' AND relname = '{$table_name}'";
    ...
    {
        try {
            $this->pdo = new PDO($this->mysql_info, array(PDO::ATTR_EMULATE_PREPARES => false));
    ...
    function query($sql)
    {
        ...
        if ($pg = $this->connection()) {
            if ($is_busy = mysql_connection_busy($pg)) exit('DB connection is busy.');
            $results = mysql_query($sql);
            $this->mysql_result_status = mysql_stat($results);
            $this->sql_error = mysql_error($pg);
        }
        if ($pg) mysql_close($pg);
    ...
    {
        if ($rs = $this->query($sql)) {
            $rows = mysql_fetch_all($rs, PGSQL_ASSOC);
    ...
    //TODO  mysql_fetch_result(): Unable to jump to row 0 on PostgreSQL result index
    /**
     * fetch result
     * 
     * @param string $sql
    ...
    {
        if ($rs = $this->query($sql)) {
            $result = mysql_fetch_result($rs, 0, 0);
    ...
    {
        if ($pg = $this->connection()) {
            if (mysql_connection_busy($pg)) {
                exit('DB connection is busy.');
            }
            $columns = [];
            $columns['id'] = '';
            $this->values = mysql_select($pg, $this->table_name, $columns, PGSQL_DML_EXEC);
            return $this;
        }
    }
     ...
     * 
     * TODO update
     * mixed mysql_update ( resource $connection , string $table_name , array $data , array $condition [, int $options = PGSQL_DML_EXEC ] )
     * 
     * @param  array $posts
     * @param  integer $id
     * @return PwPgsql
    ...
    public function copyFrom($rows)
    {
        ...
        mysql_copy_from($this->connection(), $this->table_name, $rows);
    }
    ...
    {
        $model_columns = array_keys($this->columns);
        mysql_insert($this->connection(), $this->table_name, $posts);
    }
    ...
    */
     ...
     * insertSql
     *
     * TODO : mysql_prepare, mysql_execute
     * 
     * @return string
     */
    ...
    {
        $connection = $this->connection();
        $values = mysql_meta_data($connection, $table_name);
        return $values;
    }
    ...
    {
        $connection = $this->connection();
        $dbname = mysql_dbname($connection);
        return $dbname;
    }

    /**
     * mysql_class array with attribute
     *
     * @return array
     **/
    public function pgClassArrayByConstraints($mysql_class, $mysql_constraints)
    {
        $relids[$mysql_class['mysql_class_id']] = $mysql_class['mysql_class_id'];
        foreach ($mysql_constraints as $contype => $_mysql_constraints) {
            foreach ($_mysql_constraints as $mysql_constraint) {
                if ($mysql_constraint['confrelid'] > 0) $relids[$mysql_constraint['confrelid']] = $mysql_constraint['confrelid'];
            }
        }
    ...
    /**
     * mysql_classes array with attribute, constraint
     * 
     * @param int[] mysql_class_id
     * @return array
     **/
    public function pgClassesArray($mysql_class_ids = null)
    {
        $mysql_classes = $this->pgClasses($mysql_class_ids);
        if (!$mysql_classes) return;
        $table_comments = $this->tableCommentsArray();

        foreach ($mysql_classes as $mysql_class) {
            $table_name = $mysql_class['relname'];
            $is_numbering = self::isNumberingName($table_name);
            if ($table_name && $mysql_class['mysql_class_id'] && !$is_numbering) {
                $mysql_class['comment'] = $table_comments[$table_name];
                $mysql_attributes = $this->pgAttributes($table_name);
                if ($mysql_attributes) {
                    $attributes = null;
                    $column_comments = $this->columnCommentArray($table_name);

                    $names = null;
                    foreach ($mysql_attributes as $mysql_attribute) {
                        if ($mysql_attribute['attnum'] > 0) {
                            $mysql_attribute['comment'] = $column_comments[$mysql_attribute['attname']];
                            $attributes[] = $mysql_attribute;
                        }
                    }
                }
                $mysql_class['mysql_attribute'] = $attributes;
                $mysql_class['mysql_constraint']['primary'] = $this->pgConstraints($mysql_class['mysql_class_id'], 'p');
                $mysql_class['mysql_constraint']['unique'] = $this->pgConstraints($mysql_class['mysql_class_id'], 'u');
                $mysql_class['mysql_constraint']['foreign'] = $this->pgForeignConstraints($mysql_class['mysql_class_id']);
    ...
    /**
     * mysql_class array with attribute, constraint
     * 
     * @param int mysql_class_id
     * @return array
     **/
    public function pgClassArray($mysql_class_id)
    {
        $mysql_class = $this->pgClass($mysql_class_id);
        if (!$mysql_class) return;

        $table_comment = $this->tableComment($mysql_class['relname']);
        $mysql_class['comment'] = $table_comment['description'];
        $mysql_attributes = $this->pgAttributes($mysql_class['relname']);

        if ($mysql_attributes) {
            $attributes = null;
            $column_comments = $this->columnCommentArray($mysql_class['relname']);
        ...
        $mysql_class['mysql_constraint']['primary'] = $this->pgConstraints($mysql_class['mysql_class_id'], 'p');
        //$mysql_class['mysql_constraint']['unique'] = $this->pgConstraints($mysql_class['mysql_class_id'], 'u');
        $mysql_class['mysql_constraint']['unique'] = $this->pgUniqueConstraints($mysql_class['mysql_class_id']);
        $mysql_class['mysql_constraint']['foreign'] = $this->pgForeignConstraints($mysql_class['mysql_class_id']);
    ...
    {
        $mysql_classes = $this->pgClasses();
        $comments = $this->tableCommentsArray();

        if ($mysql_classes) {
            foreach ($mysql_classes as $mysql_class) {
                if ($mysql_class['mysql_class_id']) {
                    $mysql_class['comment'] = $comments[$mysql_class['relname']];
                    $values[$mysql_class['relname']] = $mysql_class;
                }
            }
        }
        return $values;
     ...
     * 
     * TODO: mysql_field_table ?
     * TODO: mysql_field_type_oid ?
     * TODO: mysql_field_type ?
     * 
     * @param string $table_name
     * @return array
     **/
    public function attributeArray($table_name)
    {
        ...
        if ($mysql_attributes) {
            foreach ($mysql_attributes as $mysql_attribute) {
                if ($mysql_attribute['attnum'] > 0) {
                    $mysql_attribute['comment'] = $column_comments[$mysql_attribute['attname']];
                    $values[$mysql_attribute['attnum']] = $mysql_attribute;
                }
            }
        }
        return $values;
    ...
    function pgDatabases($conditions = null)
    {
        $this->dbname = null;
        $this->loadDBInfo();
        //$sql = "SELECT * FROM mysql_database WHERE datacl IS NULL;";
        $conditions[] = 'datistemplate = false';
        $where = implode(' AND ', $conditions);
        $sql = "SELECT * FROM mysql_database WHERE {$where};";
        return $this->fetchRows($sql);
    }
    ...
    function pgDatabase($name = null)
    {
        ...
        $sql = "SELECT * FROM mysql_database WHERE datname = '{$name}';";
        return $this->fetchRow($sql);
    }
    ...
    {
        $sql = "SELECT * FROM mysql_tables WHERE schemaname = '{$schema_name}' ORDER BY tablename;";
        return $this->fetchRows($sql);
    }
    ...
    {
        $sql = "SELECT mysql_class.oid AS mysql_class_id, * FROM mysql_class 
        LEFT JOIN mysql_tables ON mysql_tables.tablename = mysql_class.relname
        LEFT JOIN mysql_namespace ON mysql_namespace.oid = mysql_class.relnamespace
     ...
     * @return array
     **/
    function pgClass($mysql_class_id, $conditions = null, $relkind = 'r', $schema_name = 'public')
    {
        $sql = "SELECT mysql_class.oid AS mysql_class_id, * FROM mysql_class 
        LEFT JOIN mysql_tables ON mysql_tables.tablename = mysql_class.relname
        LEFT JOIN mysql_namespace ON mysql_namespace.oid = mysql_class.relnamespace";
        ...
        $conditions[] = "mysql_class.oid = {$mysql_class_id}";
     ...
     * f: foreign table
     *
     * @param array $mysql_class_ids
     * @param array $conditions
     * @param string $relkind
     * @param string $schema_name
     * @return array
     **/
    function pgClasses($mysql_class_ids = null, $conditions = null, $relkind = 'r', $schema_name = 'public')
    {
        $sql = "SELECT mysql_class.oid AS mysql_class_id, * FROM mysql_class 
        LEFT JOIN mysql_tables ON mysql_tables.tablename = mysql_class.relname
        LEFT JOIN mysql_namespace ON mysql_namespace.oid = mysql_class.relnamespace";
        ...
        $conditions[] = "nspname = '{$schema_name}'";
        if ($mysql_class_ids) {
            $mysql_class_id = implode(',', $mysql_class_ids);
            $conditions[] = "mysql_class.oid in ({$mysql_class_id})";
        }
        $condition = $this->sqlConditions($conditions);
        $sql .= " WHERE {$condition}";
        $sql .= " ORDER BY relname;";
     ...
     * @return array
     **/
    function pgClassById($mysql_class_id, $relkind = 'r', $schema_name = 'public')
    {
        $sql = "SELECT * FROM mysql_class 
        LEFT JOIN mysql_tables ON mysql_tables.tablename = mysql_class.relname
        LEFT JOIN mysql_namespace ON mysql_namespace.oid = mysql_class.relnamespace
        WHERE relkind = '{$relkind}' 
        AND nspname = '{$schema_name}'
        AND mysql_class.oid = '{$mysql_class_id}';";
        return $this->fetchRow($sql);
    }

    /**
     * mysql_tables (table_name)
     *
     * @param string $table_name
     * @return array
     **/
    ...
    {
        if (!$table_name) return;
        $sql = "SELECT * FROM mysql_tables WHERE schemaname = '{schema_name}' AND tablename = '{$table_name}';";
        return $this->fetchRow($sql);
    }
    ...
    {
        $sql = "SELECT mysql_class.oid AS mysql_class_id, * FROM mysql_class 
                LEFT JOIN mysql_attribute ON mysql_class.oid = mysql_attribute.attrelid
                LEFT JOIN information_schema.columns ON information_schema.columns.table_name = mysql_class.relname
                AND information_schema.columns.column_name = mysql_attribute.attname 
                WHERE mysql_attribute.attnum > 0
                AND atttypid > 0";

        if ($table_name) $sql .= " AND relname = '{$table_name}'";
        $sql .= ' ORDER BY mysql_attribute.attname;';
        return $this->fetchRows($sql);
    }
    ...
    function pgAttributeByAttnum($mysql_class_id, $attnum, $schema_name = 'public')
    {
        ...
        if (!$attnum) return;
        $sql = "SELECT * FROM mysql_attribute
        WHERE attnum > 0
        AND attnum = '{$attnum}'
        AND attrelid = '{$mysql_class_id}';";
        return $this->fetchRow($sql);
    }
     ...
     * @return array
     **/
    function pgAttributeByAttrelid($mysql_class_id)
    {
        if (!$mysql_class_id) return;
        $sql = "SELECT * FROM mysql_attribute WHERE attrelid = {$mysql_class_id};";
        return $this->fetchRows($sql);
    }
     ...
     * @return array
     **/
    function pgAttributeByIdName($mysql_class_id, $attname)
    {
        if (!$mysql_class_id) return;
        if (!$attname) return;
        $sql = "SELECT * FROM mysql_attribute WHERE attrelid = {$mysql_class_id} AND attname = '{$attname}';";
        return $this->fetchRow($sql);
    }

    /**
     * diff mysql_attribute
     *
     * @param int $mysql_class_id
     * @param string $attnum
     * @return array
    ...
    {
        $orign_mysql_attributes = $orign_pgsql->pgAttributes($orign_table_name);
        foreach ($orign_mysql_attributes as $orign_mysql_attribute) {

        }
        $mysql_attributes = $this->pgAttributes($this->name);
    }
    ...
    {
        if (!$table_name) return;
        $sql = "SELECT mysql_stat_all_tables.relname, mysql_description.description
                FROM mysql_stat_all_tables, mysql_description
                WHERE mysql_stat_all_tables.relname='\"{$table_name}\"'";
        return $this->fetchRow($sql);
    }
    ...
    function pgColumnComments()
    {
        $sql = "SELECT psat.relname, pa.attname, pd.description
                FROM mysql_stat_all_tables psat ,mysql_description pd ,mysql_attribute pa
    ...
    function pgColumnComment($table_name)
    {
        $sql = "SELECT psat.relname, pa.attname, pd.description
                FROM mysql_stat_all_tables psat ,mysql_description pd ,mysql_attribute pa
                WHERE psat.schemaname = (SELECT schemaname FROM mysql_stat_user_tables WHERE relname = '{$table_name}')
     ...
     * @return array
     */
    function pgConstraints($mysql_class_id, $type = null, $conditions = null)
    {
        if (!$mysql_class_id) return;

        $sql = "SELECT * FROM mysql_constraint";
        $sql.= " LEFT JOIN mysql_attribute ON";
        $sql.= " mysql_constraint.conrelid = mysql_attribute.attrelid";
        $sql.= " AND mysql_attribute.attnum = ANY(mysql_constraint.conkey)";

        if ($mysql_class_id) $conditions[] = "mysql_constraint.conrelid = '{$mysql_class_id}'";
        if ($type) $conditions[] = "mysql_constraint.contype = '{$type}'";
     ...
     * @return array
     */
    function pgConstraintByAttnum($mysql_class_id, $attnum, $type = null)
    {
        if (!$mysql_class_id) return;
        ...
        $conditions[] = "mysql_constraint.conrelid = '{$mysql_class_id}'";
        $conditions[] = "mysql_attribute.attnum = '{$attnum}'";
        if ($type) $conditions[] = "mysql_constraint.contype = '{$type}'";
     ...
     * @return array
     */
    function pgUniqueConstraints($mysql_class_id)
    {
        $constraints = $this->pgConstraints($mysql_class_id, 'u');
     ...
     * @return array
     */
    function pgForeignConstraints($mysql_class_id)
    {
        if (!$mysql_class_id) return;

        $sql = "SELECT 
                origin.*,
                mysql_class.oid as foreign_class_id,
                mysql_class.relname as foreign_relname,
                mysql_attribute.attnum as foreign_attnum,
                mysql_attribute.attname as foreign_attname
                FROM 
                (
                SELECT 
                    mysql_constraint.conrelid
                    , mysql_constraint.conname
                    , mysql_class.oid as mysql_class_id
                    , mysql_class.relname
                    , mysql_attribute.attnum
                    , mysql_attribute.attname
                    , mysql_constraint.confrelid
                    , mysql_constraint.confkey
                    , mysql_constraint.contype
                    , mysql_constraint.confupdtype
                    , mysql_constraint.confdeltype
                    FROM  mysql_constraint
                    LEFT JOIN mysql_attribute ON mysql_constraint.conrelid = mysql_attribute.attrelid 
                    AND mysql_attribute.attnum = ANY(mysql_constraint.conkey)
                    LEFT JOIN mysql_class ON mysql_constraint.conrelid = mysql_class.oid
                    WHERE mysql_constraint.contype = 'f' AND mysql_constraint.conrelid = '{$mysql_class_id}'
                ) AS origin
                LEFT JOIN mysql_attribute ON origin.confrelid = mysql_attribute.attrelid 
                AND mysql_attribute.attnum = ANY(origin.confkey)
                LEFT JOIN mysql_class ON origin.confrelid = mysql_class.oid
                ;";
    ...
    {
        $sql = "SELECT * FROM mysql_constraint WHERE conname LIKE '%{$name}%';";
        return $this->fetchRows($sql);
    }
     ...
     *
     * @param  array $mysql_class
     * @return array
     */
    function pgConstraintGroup($mysql_class)
    {
        $constraints = $this->pgConstraintsByPgClassID($mysql_class['mysql_class_id']);
        if (!$constraints) return;
    ...
    /**
     * pg constraint by mysql_class
     *
     * @param  string $table_name
     * @return array
     */
    function pgConstraintsByPgClassID($mysql_class_id)
    {
        $constraints = $this->pgConstraints($mysql_class_id);
        if (!$constraints) return;
    ...
    {
        $mysql_class = $this->pgClassByRelname($table_name);
        $constraints = $this->pgConstraints($mysql_class['mysql_class_id'], $type);
        if (!$constraints) return;
    ...
    /**
     * set Model(table name, columns ...) By mysql_attributes
     *
     * @param  string $model_name
     * @return PwPgsql
     */
    function table($table_name)
    {
        ...
        $mysql_attributes = $this->pgAttributes($this->name);
        if (!$mysql_attributes) return;

        foreach ($mysql_attributes as $mysql_attribute) {
            if ($mysql_attribute['attname'] != $this->id_column) {
                $value = null;
                $value['type'] = $mysql_attribute['udt_name'];
                if ($mysql_attribute['attnotnull'] == 't') {
                    $value['is_required'] = true;
                }
                if ($mysql_attribute['attname']) {
                    $columns[$mysql_attribute['attname']] = $value;
                }
            }
        }
    ...
    public function diffFromVoModel()
    {
        ...
        foreach (glob($model_path) as $model_file) {
            ...
            if (class_exists($path_info['filename'])) {
                ...
                $mysql_class = $this->pgClassByRelname($model->name);
            }
            if (!$mysql_class) {
                $sql = $this->createTableSql($model);
                ...
                $mysql_attributes = null;
                if ($attributes = $this->pgAttributes($model->name)) {
                    foreach ($attributes as $mysql_attribute) {
                        if ($mysql_attribute['attname']) {
                            $mysql_attributes[$mysql_attribute['attname']] = $mysql_attribute;
                        }
                    }
                }
                if ($mysql_attributes) {
                    foreach ($model->columns as $column_name => $column) {
                        $status = '';
                        $attribute = $mysql_attributes[$column_name];
                        if (!$attribute) {
